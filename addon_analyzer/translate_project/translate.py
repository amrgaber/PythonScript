...
def parse_arguments():
    ...
    parser.add_argument("-p", "--path", required=True,
                        help="Path to the source PO file (can be relative or absolute, may include sub-folder).")
...
def translate_po_file(src_path, target_lang, dest_dir=None, create_dir=False, out_format="{base_name}_{date}_{time}.po"):
    ...
    if not os.path.exists(dest_dir) and not create_dir and not dest_dir:
        raise Exception(
            f"Destination directory '{dest_dir}' does not exist. Use -c to create it or specify a different -d option.")
...
def main():
    ...
    translated_path = translate_po_file(
        src_path, target_lang, dest_dir, create_dir, args.out_format)
